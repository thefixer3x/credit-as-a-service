...
import { createLedgerRoutes } from './routes/ledger-routes.js';
...
async function startServer() {
  try {
    const app = express();
    const PORT = env.LEDGER_SERVICE_PORT || 3008;
    ...
    // Initialize services
    const ledgerEngine = new LedgerEngine(cacheService);
    const controller = new LedgerController(ledgerEngine);
    ...
    // API routes
    app.use('/api/v1/ledger', createLedgerRoutes(controller));
